...
useEffect(() => {
  if (propertyId) {
    const fetchProperty = async () => {
      try {
        const { data } = await axios.get(`https://backend-ppt.onrender.com/api/properties/listing/${propertyId}`);
        setFormData({ ...data.property });
        ...
        if (data.property.propertyImages?.length) {
          const previews = data.property.propertyImages
            ...
            .map(img => ({
              url: img.startsWith('http') ? img : `https://backend-ppt.onrender.com/${img}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://backend-ppt.onrender.com/api/editProperty/${propertyId}`, data, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      alert('Property updated successfully');
      router.push('/edit-listing');
